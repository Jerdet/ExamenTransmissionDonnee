...
unsigned volatile char TabData[5];     //variable temporaire pour afficher des valeurs au lcd
...
/*
 * APPEL : initADC();
 * Descr : Premet de configurer le convertisseur AD
 * INPUT : N/A
 * OUTPUT: N/A
 */
void getTemperature(void);
/*
 * APPEL : getTemperature();
 * Descr : Dépendant de la variable [etatTemperature] effectue :
 *          le démarrage de la conversion de la temperature
 *          L'enregistrement de la temperature
 *              Via le capteur one wire
 * INPUT : N/A
 * OUTPUT: N/A
 */
...
/*
 * APPEL : getConversion();
 * Descr : Dépendant de la variable [etatConversion] effectue :
 *          -Lancement de la conversion
 *          -Enregistrement de la valeur convertie
 *              Du canal 24 pour la lumière
 * INPUT : N/A
 * OUTPUT: N/A
 */
void getPcUsart(char *caractere);
/*
 * APPEL : getPcUsart([TABCARAC]);
 * Descr : Est appeler par les callbacks lorsque les info du pc on été enregistrer
 * INPUT : TABCARAC : est un tableau de caractère qui contient les données de l'usart venant du pc
 * OUTPUT: N/A
 */
...
void rfidLect(char sec)
{
    ...
    {
        while(TXSTA2bits.TRMT != 1);        //vérifie si le buffer d'envoie est vide, 0 = plein, 1 = vide
        TXREG2 = rfid[tmp];
        tmp++;
    }
}
...
{
    if(bt == 'e')           //le bouton du milieu a été appuyer
    {
        rfidLect(0x02);     //on lance la lecture de la carte RFID
    }
...
{
    if(data1 == 0 && data2 == 0 && data3 == 0 && data4 == 0)        //Si une carte a été déteter
    {
        SetDDRamAddr(0x40);                 //
        ...
        SetDDRamAddr(0x40);
        while(BusyXLCD());
        putrsXLCD("Pas de carte");          //on signale qu'aucune carte n'a ete trouver
    }
    else                                    //une carte a été trouver est les donénes sont fournie
    {
        TabData[0] = data1;     //stockage des données dans un tableau pour une meilleur manipulation
        TabData[1] = data2;
        TabData[2] = data3;
        TabData[3] = data4;
        TabData[4] = 0;         //permet de signaler a la fonction putsXLCD quand s'arrêter
        SetDDRamAddr(0x40);                 //
        while(BusyXLCD());                  // nettoyage de la ligne du bas de l'LCD
        putrsXLCD("                ");      //
        while(BusyXLCD());
        SetDDRamAddr(0x40);
        while(BusyXLCD());
        putsXLCD(TabData);                  //affichage des données recu de l'RFID
...
void InitADC(void){
    ADCON0bits.CHS=0b11000;     //canal 24
    ADCON0bits.ADON=1;          //active le module
    ADCON2bits.ADFM=1;          //justification a droite
    ADCON1bits.PVCFG=0b00;      //selection de la tension de référence positive, ici , les 5V de la carte
    ADCON1bits.NVCFG=0b00;      //selection de la tension de référence négative, ici , le ground de la carte
    PMD2bits.ADCMD=0;           //acivation du module ADC
    ADCON2bits.ADCS=0b101;      //selectoin de la fréquence ici Fosc/16
    ADCON2bits.ACQT=0b010;      //selection du temps TAD ici 4 TAD
}

void getConversion(void){
    if(etatConversion == 0){
        ADCON0bits.GO_DONE=1;       //lance la conversion AN
        etatConversion = 1;
    }else{
        //verifier quand même que la conversion est terminer
        adcRec = ((unsigned int)ADRESH<<8)|(ADRESL);    //enregistre la valeur dans une variable
        adc = (((float)adcRec)*5.0)/1024;               //transformation en pourcentage
        ftoa(adc, tamponAdc, 2, 'f');                   //conversion de numérique a ascii
        etatConversion = 0;
    }
...
void getTemperature(void){
    ...
    {
        Convert_Temperature();              //lancement de la conversion de temperature via le one wire
        etatTemperature = 1;
    }
    else
    {
...
void getPcUsart(char *caractere){
    int compteurUsart=0;
    SetDDRamAddr(0x40);                 //
    while(BusyXLCD());                  //nettoyage de la ligne du bas du LCD
    putrsXLCD("                ");      //
    while(BusyXLCD());
    SetDDRamAddr(0x40);
    while(BusyXLCD());
    putsXLCD(&caractere[0]);            //on affiche se qui est recu de l'usart sur l'LCD
    rfidEcri(0x02,caractere[0],caractere[1],caractere[2],caractere[3]);     //on envoi les données sur une carte RFID
    for(compteurUsart=0;compteurUsart<5;compteurUsart++)                    //on enttoye le tableau caractere
